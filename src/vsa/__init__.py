...
class HDVector:
    ...
    def __mul__(self, other):
        vector = np.fft.irfft(np.fft.rfft(self._vector) * np.fft.rfft(other._vector),
                              n=self.dimension)
        return HDVector(vector=vector)

    def __add__(self, other):
        vector = self._vector + other._vector
...
class ItemMemory():
    ...
    def get_vector(self, key: str, create=True) -> HDVector:
